...
function generateAxiosAuthFile(config: GeneratorConfig): string {
  const isTypeScript = config.language === 'ts';
...
function generateFetchAuthFile(config: GeneratorConfig): string {
  const isTypeScript = config.language === 'ts';
...
function generateFunctionName(requestName: string): string {
  return requestName
    .replace(/[^a-zA-Z0-9]/g, ' ')
    .split(' ')
...
export function generateBaseUrlFile(
  config: GeneratorConfig,
  collection: ParsedCollection
): string {
  const isTypeScript = config.language === 'ts';
...
export function generateInstanceFile(
  config: GeneratorConfig,
  collection: ParsedCollection
): string {
  const isTypeScript = config.language === 'ts';
...
export function generateAuthFunctionsFile(
  config: GeneratorConfig,
  collection: ParsedCollection
): string {
  const isTypeScript = config.language === 'ts';
  ...
  const authFolder = collection.folders.find(
    folder =>
      folder.name.toLowerCase().includes('auth') ||
      folder.name.toLowerCase().includes('login')
  );
...
export function generateApiFunctionsFile(
  config: GeneratorConfig,
  folder: PostmanFolder,
  collection: ParsedCollection
): string {
...
function generateFunctionWithTryCatch(
  config: GeneratorConfig,
  request: PostmanRequest,
  functionName: string,
  hasBody: boolean,
  ...
  if (useAxios) {
    let axiosOptions = '';
    if (hasBody) axiosOptions += ', { data }';
    else if (hasQuery) axiosOptions += ', { params }';
    else if (hasBody && hasQuery) axiosOptions += ', { data, params }';
